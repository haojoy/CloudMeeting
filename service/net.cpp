...
int	Tcp_connect(const char * host, const char * serv)
{
    ...
    {
        QUIT_ERR("tcp_connect error for %s, %s: %s", host, serv, gai_strerror(n));
    }
    ...
    if(res == NULL)
    {
        QUIT_ERR("tcp_connect error for %s,%s", host, serv);
    }
...
int Tcp_listen(const char * host, const char * service, socklen_t * addrlen)
{
    ...
    {
        QUIT_ERR("tcp listen error for %s %s: %s", host, service, gai_strerror(n));
    }
	ressave = res;
	do
	{
        ...
        {
            MSG_INFO("server address: %s", Sock_ntop(addr, MAXSOCKADDR, res->ai_addr, res->ai_addrlen));
            break; //success
        }
        Close(listenfd);
	}while((res = res->ai_next) != NULL);
    ...
    if(res == NULL)
    {
        QUIT_ERR("tcp listen error for %s: %s", host, service);
    }
...
ssize_t read_fd(int fd, void *ptr, size_t nbytes, int *recvfd)
{
    ...
    if((cmptr = CMSG_FIRSTHDR(&msg)) != NULL && cmptr->cmsg_len == CMSG_LEN(sizeof(int)))
    {
        ...
        {
            MSG_ERR("control level != SOL_SOCKET");
        }
        if(cmptr->cmsg_type != SCM_RIGHTS)
        {
            MSG_ERR("control type != SCM_RIGHTS");
        }
