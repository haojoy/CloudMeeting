...
#define QUIT_ERR(fmt, ...) err_quit(__FILE__, __LINE__, __FUNCTION__, ERROR, fmt, ##__VA_ARGS__)
#define QUIT_INFO(fmt, ...) err_quit(__FILE__, __LINE__, __FUNCTION__, INFORMATION, fmt, ##__VA_ARGS__)
#define MSG_ERR(fmt, ...) err_msg(__FILE__, __LINE__, __FUNCTION__, ERROR, fmt, ##__VA_ARGS__)
#define MSG_INFO(fmt, ...) err_msg(__FILE__, __LINE__, __FUNCTION__, INFORMATION, fmt, ##__VA_ARGS__)
#define PRTNT_CONTEXT_HEX(sendbuf, len)\
   do{ \
        char printbuf[3*len+1];\
        convert_to_hex_string(sendbuf,len,printbuf,3*len+1);\
        MSG_INFO("sendbuf content (hex): %s", printbuf);\
   }while(0);

#define PRTNT_IP_STR(ip_uint32)\
   do{\
      char ip_str[INET_ADDRSTRLEN];\
      ipConvertToString(ip_uint32, ip_str, sizeof(ip_str));\
      MSG_INFO("ip: %s ",ip_str);\
   }while(0);
