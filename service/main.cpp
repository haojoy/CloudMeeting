...
int main(int argc, char **argv)
{
    ...
    {
        QUIT_ERR("usage: ./app [host] <port #> <#threads> <#processes>");
    }
    maxfd = listenfd;
    ...
    MSG_INFO("total threads: %d  total process: %d", nthreads, nprocesses);
    ...
    for(;;)
    {
        ...
        for(i = 0; i < nprocesses; i++)
        {
            if(FD_ISSET(room->pptr[i].child_pipefd, &rset))
            {
                ...
                {
                    QUIT_ERR("child %d terminated unexpectedly", i);
                }
				MSG_INFO("c = %c", rc);
                if(rc == 'E') // room empty
                {
                    ...
                    room->navail++;
                    MSG_INFO("room %d is now free", room->pptr[i].child_pid);
                ...
                {
                    MSG_ERR("read from %d error", room->pptr[i].child_pipefd);
                    continue;
                }
